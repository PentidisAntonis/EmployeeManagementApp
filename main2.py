...
class DatabaseConnection:
    def __init__(self, host="localhost", user="ahpentidis", password="P@ssw0rd", database="project"):
        self.host = host
        self.user = user
    ...
    def connect(self):
        try:
            ...
            return self.connection  # Return the connection object if successful
...
class EditDialog(QDialog):
    ...
    def update_employee(self):
        connection = main_window.db_connection.connect()
        cursor = connection.cursor()
        cursor.execute("UPDATE employees SET name = %s, department = %s, mobile = %s WHERE employee_id = %s",
...
class InsertDialog(QDialog):
    ...
    def add_employee(self):
        try:
            ...
            department = self.department_name.currentText()
            ...
            try:
                cursor.execute("INSERT INTO employees (name, department, mobile) VALUES (%s, %s, %s)",
                               (name, department, mobile))
                connection.commit()
                print("Employee inserted successfully.")

                # Reload data in the main window table view
                main_window.load_data()
...
class SearchDialog(QDialog):
    ...
    def search(self):
        ...
        main_window.table.setRowCount(0)  # Clear the existing contents of the table
        for row_number, row_data in enumerate(result):
